        ...
        <div className="flex-row justify-between items-baseline">
          <div>
            <div className="flex-row items-center gap-2">
              <p className="subtitle m-0">ASC - Anesthesia & Surgical Care</p>
              <span className={`w-2 h-2 rounded-full ${isOnline ? 'bg-success' : 'bg-error'}`}></span>
            </div>
            <h2 className="title">Surgery Command Center</h2>
          </div>
        ...
        <div className="mt-8">
          ...
          <div className="flex-column gap-4">
            {isLoading ? (
              ...
              <ClinicalCard key={item.id} className="p-6 flex-row items-center gap-8 hover:shadow-lg transition-all">
                <div className={`w-16 h-16 rounded-2xl bg-${getStatusColor(item.status)}/10 flex items-center justify-center text-${getStatusColor(item.status)}`}>
                  <span className="material-symbols-outlined text-3xl">precision_manufacturing</span>
                </div>
                <div className="flex-1">
                  <div className="flex-row items-center gap-3">
                    <h4 className="text-lg font-black">{item.patient_name}</h4>
                    <span className={`chip chip-${getStatusColor(item.status)} text-[8px] font-black uppercase`}>{item.status}</span>
                  </div>
                  <p className="text-xs font-bold opacity-60 uppercase mt-1">{item.procedure_name} • OR {item.room_number}</p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] font-black uppercase opacity-40">Surgeon</p>
                  <p className="text-sm font-black">Dr. {item.surgeon_name}</p>
                </div>
                <div className="w-px h-12 bg-outline-variant" />
                <div className="flex-column items-end min-w-[120px]">
