...
export default function PatientIdentityCard({ patient, onOpenNewEncounter, onOpenReconciliation }) {
  ...
  if (!patient) {
    return (
      <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-center flex flex-col items-center justify-center gap-2">
        <span className="material-symbols-outlined text-slate-300 dark:text-slate-700 text-[40px]">person_off</span>
        <h3 className="text-sm font-bold text-slate-700 dark:text-slate-300">Tidak Ada Pasien Aktif</h3>
        <p className="text-xs text-slate-400">Pilih pasien dari daftar pencarian untuk memuat kartu identitas klinis.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm flex flex-col gap-4 relative overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-4">
        <div className="flex items-center gap-3.5">
          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-lg text-white shadow-md ${
            isEmergencyAnon ? 'bg-rose-600 animate-pulse' : 'bg-blue-600'
          }`}>
